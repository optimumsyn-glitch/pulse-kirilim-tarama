...
if not st.session_state.authenticated:
    ...
    girilen_sifre = st.text_input("Şifreyi girin", type="password")
    if st.button("Giriş Yap"):
        if girilen_sifre == "pulse2026":
            st.session_state.authenticated = True
            st.success("Giriş başarılı! Tarama ekranı yükleniyor...")
            st.rerun()
        else:
            st.error("Yanlış şifre!")

# Şifre doğruysa çalışacak ana gövde
if st.session_state.authenticated:
    ...
    timeframe = st.sidebar.selectbox("⏰ Zaman Dilimi", ["1s", "4s", "Günlük", "Haftalık", "Aylık"], index=2)
    ...
    st.title("📊 Pulse Kırılım Tarayıcı")
    st.write(f"Zaman Dilimi: **{timeframe}** | Liste Derinliği: **{len(bist_symbols)} Hisse**")
    ...
    if st.button(f"🚀 {timeframe} Tarama Başlat", type="primary"):
        ...
        for i, symbol in enumerate(bist_symbols):
            try:
                # KRİTİK DÜZELTME: Veriyi çekerken yfinance hatası oluşmaması için yapılandırıldı
                ...
                # ÇOK ÖNEMLİ: yfinance'ın yeni Multi-Index başlık yapısı düzleştiriliyor (KeyError'u çözen yer)
                if isinstance(data.columns, pd.MultiIndex):
                    data.columns = data.columns.get_level_values(0)

                # Son kapanış fiyatı güvenli bir şekilde çekiliyor
                last_close_val = data['Close'].iloc[-1]
                current_close = float(last_close_val.iloc[0] if isinstance(last_close_val, pd.Series) else last_close_val)
                ...
                if result:
                    ...
                    live_found_text.success(f"🔥 SİNYAL: **{result['Sembol']}** | Güç: {result['Güç']} | Konum: {result['Yakınlık']}")

                progress_bar.progress((i + 1) / len(bist_symbols))
                status_text.info(f"İşlenen Hisse: {i+1}/{len(bist_symbols)} | Bulunan Sinyal: {len(results)}")

            except Exception as e:
                # Arka planda bir hata olursa tarama donmasın, sıradaki hisseye geçsin
                continue
                
            if sleep_time > 0:
                time.sleep(sleep_time)

        # Durum çubuklarını temizle
        status_text.empty()
        ...
        if results:
            ...
            st.success(f"✅ Tarama başarıyla bitti! Koşulları karşılayan {len(df)} hisse bulundu.")
            st.dataframe(df, use_container_width=True)

            # Excel Formatlama İşlemleri
            output = io.BytesIO()
            wb = Workbook()
            ws = wb.active
            ws.title = "Kırılım Raporu"
            ...
            st.download_button(
                label="📥 Excel Sinyal Listesini İndir",
                data=output,
                file_name=f"Pulse_Kirilim_{tf['name']}_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            )
        else:
            st.warning(f"Seçilen periyotta ('{timeframe}') kriterlere uyan hiçbir kırılım sinyali bulunamadı.")
