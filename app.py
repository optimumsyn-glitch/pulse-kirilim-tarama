...
# ==================== BASİT VE GÜVENLİ OTURUM HAFIZASI ====================
if "authenticated" not in st.session_state:
    st.session_state.authenticated = False

# ==================== KİLİTLENMEYEN ŞİFRE KONTROLÜ ====================
if not st.session_state.authenticated:
    st.title("🔐 Pulse Kırılım Tarayıcı")
    st.markdown("**Abone Özel** — Direnç Kırılımı Sinyalleri")
    
    # Kilitlenmeye sebep olan form ve rerun yapıları kaldırıldı, düz inputa geçildi
    girilen_sifre = st.text_input("Şifreyi girin", type="password")
    if st.button("Giriş Yap"):
        if girilen_sifre == "pulse2026":
            st.session_state.authenticated = True
            st.success("Giriş başarılı! Lütfen sayfayı bir kez hareket ettirin veya bekleyin.")
            # st.rerun() KULLANILMADI - Kilitlenme önlendi
        else:
...
if st.session_state.authenticated:
    ...
    st.title("📊 Pulse Kırılım Tarayıcı")
    st.write(f"Mevcut Seçim: **{timeframe}** | Aktif Liste: **{len(bist_symbols)} Hisse**")
    ...
    # ==================== TARAMA TETİKLEYİCİ BUTONU ====================
    # Düz buton yapısı: Butona basıldığı an altındaki kodlar kilitlenmeden akar.
    if st.button(f"🚀 {timeframe} Tarama Başlat", type="primary"):
        ...
        for i, symbol in enumerate(bist_symbols):
            try:
                ...
                status_text.info(f"İşlenen: {i+1}/{len(bist_symbols)} Hisse | Sinyal: {len(results)}")
        ...
        if results:
            ...
            st.success(f"✅ Tarama tamamlandı! {len(df)} adet güçlü kırılım saptandı.")
            st.dataframe(df, use_container_width=True)

            # Excel Hazırlama Modülü
            output = io.BytesIO()
            wb = Workbook()
            ws = wb.active
            ...
            st.warning(f"Seçilen periyotta strateji kriterlerine uyan bir kırılım bulunamadı.")
